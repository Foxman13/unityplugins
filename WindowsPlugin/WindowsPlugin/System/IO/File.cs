...
namespace LegacySystem.IO
{
    public class File
    {
...
#elif WINDOWS_PHONE
            //Contract.Assert(!String.IsNullOrEmpty(path), "Parameter /path/ cannot be null or empty!");
        ...
        public static void WriteAllText(string path, string data)
        {
            ...
            thread.Wait();
#elif WINDOWS_PHONE
            //Contract.Ensures(!String.IsNullOrEmpty(path), "Parameter /path/ cannot be null or empty!");

            try
            {
                using (var sw = System.IO.File.CreateText(path))
                {
                    var task = sw.WriteAsync(data);
                    task.Wait();
                }
            }
            catch(UnauthorizedAccessException uae)
